...
class LoadAnnotations(object):
    ...
    def _load_semantic_seg(self, results):
        ...
        with open(filename, 'rb') as f:
            img_bytes = f.read()
        results['gt_semantic_seg'] = imtransform.imfrombytes(
...
class Resize(object):
    ...
    def _resize_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.keep_ratio:
                img, scale_factor = imtransform.imrescale(
                    results[key],
                    results['scale'],
                    return_scale=True,
                ...
                img, w_scale, h_scale = imtransform.imresize(
                    results[key],
                    results['scale'],
                    return_scale=True,
                    backend=self.backend)
    ...
    def _resize_seg(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.keep_ratio:
                gt_seg = imtransform.imrescale(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
            else:
                gt_seg = imtransform.imresize(
                    results[key],
                    results['scale'],
                    interpolation='nearest',
                    backend=self.backend)
...
class RandomFlip(object):
    ...
    def __call__(self, results):
        ...
        if results['flip']:
            # flip image
            for key in results.get('img_fields', ['img']):
                results[key] = imtransform.imflip(
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            if self.size is not None:
                padded_img = imtransform.impad(
                    results[key], shape=self.size, pad_val=self.pad_val)
            elif self.size_divisor is not None:
                padded_img = imtransform.impad_to_multiple(
...
class Normalize(object):
    ...
    def __call__(self, results):
        ...
        if 'img' in results.keys():
            for key in results.get('img_fields', ['img']):
                results[key] = imtransform.imnormalize(results[key], self.mean, self.std,
                                                self.to_rgb)
        if 'rgb' in results.keys():
            for i in range(results['rgb'].shape[0]):
                results['rgb'][i, :, :, :] = imtransform.imnormalize(results['rgb'][i, :, :, :], self.mean, self.std,
                                                self.to_rgb)
    ...
    def __call__(self, results):
        ...
        for key in results.get('seg_fields', []):
            if self.scale_factor != 1:
                results[key] = imtransform.imrescale(
                    results[key],
                    self.scale_factor,
                    interpolation='nearest',
                    backend=self.backend)
