...
class BitmapMasks(BaseInstanceMasks):
    ...
    def rescale(self, scale, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            new_w, new_h = imtransform.rescale_size((self.width, self.height), scale)
            rescaled_masks = np.empty((0, new_h, new_w), dtype=np.uint8)
        else:
            rescaled_masks = np.stack([
                imtransform.imrescale(mask, scale, interpolation=interpolation)
    ...
    def resize(self, out_shape, interpolation='nearest'):
        ...
        if len(self.masks) == 0:
            ...
            resized_masks = np.stack([
                imtransform.imresize(mask, out_shape, interpolation=interpolation)
                for mask in self.masks
            ])
    ...
    def flip(self, flip_direction='horizontal'):
        ...
        if len(self.masks) == 0:
            ...
            flipped_masks = np.stack([
                imtransform.imflip(mask, direction=flip_direction)
                for mask in self.masks
            ])
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            padded_masks = np.stack([
                imtransform.impad(mask, shape=out_shape, pad_val=pad_val)
                for mask in self.masks
            ])
    ...
    def translate(self,
        ...
        if len(self.masks) == 0:
            ...
            translated_masks = imtransform.imtranslate(
    ...
    def shear(self,
        ...
        if len(self.masks) == 0:
            ...
            sheared_masks = imtransform.imshear(
                self.masks.transpose((1, 2, 0)),
                magnitude,
                direction,
    ...
    def rotate(self, out_shape, angle, center=None, scale=1.0, fill_val=0):
        ...
        if len(self.masks) == 0:
            ...
            rotated_masks = imtransform.imrotate(
                self.masks.transpose((1, 2, 0)),
                angle,
                center=center,
                scale=scale,
